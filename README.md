...
Packet is actively rolling out new hardware in multiple locations which supports ESXi 6.7. Until the gen 3 hardware is more widely available, we'll not make gen 3 hardware the default but provide the option to use it.

### Costs
The gen3 [c3.medium.x86](https://www.packet.com/cloud/servers/c3-medium/) is $0.10 more than the [c2.medium.x86](https://www.packet.com/cloud/servers/c2-medium-epyc/) but benefits from higher clock speed and the storage is better utilized to create a larger vSAN data store.
...
### Known Issues
ESXi 6.7 deployed on [c3.medium.x86](https://www.packet.com/cloud/servers/c3-medium/) may result in an alarm in vCenter which states `Host TPM attestation alarm`. The Packet team is looking into this but its thought to be a cosmetic issue.
